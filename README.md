...
- **Calico CNI:** Installed as the core network provider, replacing default Flannel to support advanced `NetworkPolicy` custom resources.
...
**Test A — Insecure image (should be blocked):**

```bash
./deploy-pipeline.sh nginx:1.14
```

> **Expected:** Trivy detects multiple `CRITICAL` vulnerabilities. Pipeline exits with code `1` and halts before reaching the cluster.

**Test B — Secure image (should pass):**

```bash
./deploy-pipeline.sh nginx:alpine
```

> **Expected:** No `CRITICAL` findings. Pipeline proceeds to Stage 2.

---

### Scenario 2: Zero-Trust Network Micro-segmentation (Calico Gate)

The cluster enforces strict separation between three tiers inside the `production-env` namespace: `web-app`, `backend-app`, and `database-app`.
...
> **Expected:** Connection drops completely — persistent request timeout.

---

### Scenario 3: Real-time Incident Detection (Falco Gate)

Even if an attacker gains access inside a running container, their actions are tracked in real time.
...
> **Expected:** Falco captures `execve` and `openat` syscalls and outputs structured alerts with full container metadata.
...
Falco suffered from **container blindness** because K3s isolates its runtime at `/run/k3s/containerd/containerd.sock` instead of the global standard `/run/containerd/containerd.sock`.  
...
Automating deployments via Helm triggered connection timeouts on port `8080`.  
**Fix:** Explicitly define the `KUBECONFIG` environment variable pointing to `/etc/rancher/k3s/k3s.yaml`.

### 3. Kernel Header Dependencies
Runtime monitoring via eBPF requires direct kernel symbol mapping.  
**Fix:** Manually install `linux-headers` packages matching the active kernel version before loading the eBPF driver.